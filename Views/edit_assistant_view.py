...
class EditAssistantView(ttk.Frame):

    def __init__(self, master, control, **kargs):
        super().__init__(master, **kargs)
        ViewManager.instance.register_view(self, "EditAssistantView")

        self.control = control
    
    def display_assist_info(self, assistants, assist_id):
        ViewManager.instance.show_view("EditAssistantView")
        for widget in self.winfo_children():
            widget.destroy()

        self.selected_teacher = assistants[assist_id]
        # passing the teacher's fname and lname to get teacher's info
        self.curr_assist_info = self.control.get_teacher_info(self.selected_teacher[0], self.selected_teacher[1])
        ...
        r = 0
        each_display_box = []
        for each_info in self.curr_assist_info:
